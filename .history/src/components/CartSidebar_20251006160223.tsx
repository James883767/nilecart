...
export const CartSidebar = () => {
  ...
  useEffect(() => {
    if (items.length > 0 && !sellerWhatsApp) {
      ...
      if (sellerId) {
        console.log('Fetching WhatsApp for seller:', sellerId);
        fetchSellerWhatsApp(sellerId);
      } else {
        console.log('No seller ID found in cart items');
      }
    }
  ...
  const fetchSellerWhatsApp = async (sellerId: string) => {
    try {
      console.log('Fetching WhatsApp for seller ID:', sellerId);
      ...
      console.log('WhatsApp fetch result:', { data, error });

      if (error) {
        console.error('Database error:', error);
        if (error.code === 'PGRST116') {
          console.log('Profile not found for seller ID:', sellerId);
        }
        return;
      }

      if (data?.whatsapp_number) {
        console.log('Setting seller WhatsApp:', data.whatsapp_number);
        setSellerWhatsApp(data.whatsapp_number);
      } else {
        console.log('No WhatsApp number found for seller:', data);
        // Set a placeholder to indicate we tried but no WhatsApp available
