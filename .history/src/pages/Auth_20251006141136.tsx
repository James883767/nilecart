...
const Auth = () => {
  ...
  useEffect(() => {
    ...
    if (!authLoading && user) {
      // Only redirect if we have a profile with a valid role
      if (profile && profile.role) {
        console.log('Redirecting to:', profile.role === 'seller' ? '/dashboard' : '/home');
        navigate(profile.role === 'seller' ? '/dashboard' : '/home');
      } else {
        // If no profile or no role, wait for profile to load
        console.log('User authenticated but no profile/role yet - waiting for profile to load');
